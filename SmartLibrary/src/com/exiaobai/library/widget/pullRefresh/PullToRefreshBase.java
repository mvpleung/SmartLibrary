...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public final boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (isReadyForPullDown()) {
				...
				handled = true;
			} else if (!isPullLoading() && isReadyForPullUp()) {
				// } else if (isPullLoadEnabled() && isReadyForPullUp()) {
	...
	public void onPullDownRefreshComplete() {
		onPullDownRefreshComplete(null);
	}

	@Override
	public void onPullDownRefreshComplete(CharSequence lastUpdate) {
		if (isPullRefreshing()) {
		...
		if (!TextUtils.isEmpty(lastUpdate)) {
			if (null != mHeaderLayout) {
				mHeaderLayout.setLastUpdatedLabel(lastUpdate);
			}

			if (null != mFooterLayout) {
				mFooterLayout.setLastUpdatedLabel(lastUpdate);
			}
		}
	}
	...
	/**
	 * 开始加载，通常用于调用者主动加载，典型的情况是进入界面，开始主动加载，这个加载并不是由用户拉动引起的
	 * 
	 * @param smoothScroll
	 *            表示是否有平滑滚动，true表示平滑滚动，false表示无平滑滚动
	 * @param delayMillis
	 *            延迟时间
	 */
	public void doPullLoading(final boolean smoothScroll, final long delayMillis) {
		postDelayed(new Runnable() {
			@Override
			public void run() {
				int newScrollValue = mFooterHeight;
				int duration = smoothScroll ? SCROLL_DURATION : 0;
	...
	protected void pullFooterLayout(float delta) {
		...
		int scrollY = Math.abs(getScrollYValue());
		if (isPullLoadEnabled() && !isPullLoading() && hasMoreData()) {
	...
	public boolean hasMoreData() {
		return (mPullUpState != State.NO_MORE_DATA);
	}
