...
public class PullToRefreshListView extends PullToRefreshBase<ListView> implements OnScrollListener {
	...
	protected ListView createRefreshableView(Context context, AttributeSet attrs) {
		...
		// com.lidroid.xutils.bitmap.PauseOnScrollListener(BitmapHelper.getBitmapUtils(),
		// false, true, this));
		mListView.setOnScrollListener(new PauseOnScrollListener(BitmapHelper.getInstance().getImageLoader(), true, false, this));
	 ...
	 *            true表示还有更多的数据，false表示没有更多数据了
	 */
	public void setHasMoreData(boolean hasMoreData, CharSequence label) {
		if (!hasMoreData) {
			if (null != mLoadMoreFooterLayout) {
				mLoadMoreFooterLayout.setState(State.NO_MORE_DATA);
				mLoadMoreFooterLayout.setNoMoreLabel(label);
			}

			LoadingLayout footerLoadingLayout = getFooterLoadingLayout();
			if (null != footerLoadingLayout) {
				footerLoadingLayout.setState(State.NO_MORE_DATA);
				footerLoadingLayout.setNoMoreLabel(label);
			}
		} else {
			if (null != mLoadMoreFooterLayout) {
				mLoadMoreFooterLayout.setState(State.NONE);
