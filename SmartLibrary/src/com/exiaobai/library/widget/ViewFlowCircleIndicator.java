...
public class ViewFlowCircleIndicator extends View implements ViewFlowIndicator, AnimationListener {
	...
	private int measureSpecWidth, measureSpecHeight;
	...
	public ViewFlowCircleIndicator(Context context, AttributeSet attrs) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ViewFlowCircleIndicator);

		// Gets the inactive circle type, defaulting to "fill"
		int activeType = a.getInt(R.styleable.ViewFlowCircleIndicator_activeType, STYLE_FILL);

		int activeDefaultColor = 0xFFFFFFFF;

		// Get a custom inactive color if there is one
		int activeColor = a.getColor(R.styleable.ViewFlowCircleIndicator_activeColor, activeDefaultColor);

		// Gets the inactive circle type, defaulting to "stroke"
		int inactiveType = a.getInt(R.styleable.ViewFlowCircleIndicator_inactiveType, STYLE_STROKE);

		int inactiveDefaultColor = 0x44FFFFFF;
		// Get a custom inactive color if there is one
		int inactiveColor = a.getColor(R.styleable.ViewFlowCircleIndicator_inactiveColor, inactiveDefaultColor);

		// Retrieve the radius
		radius = a.getDimension(R.styleable.ViewFlowCircleIndicator_radius, 4.0f);

		circleSeparation = a.getDimension(R.styleable.ViewFlowCircleIndicator_circleSeparation, 2 * radius + radius);
		activeRadius = a.getDimension(R.styleable.ViewFlowCircleIndicator_activeRadius, 0.5f);
		// Retrieve the fade out time
		fadeOutTime = a.getInt(R.styleable.ViewFlowCircleIndicator_fadeOut, 0);

		mCentered = a.getBoolean(R.styleable.ViewFlowCircleIndicator_centered, false);
	...
	public void reMeasureWidthHeight() {
		setMeasuredDimension(measureWidth(measureSpecWidth), measureHeight(measureSpecHeight));
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		this.measureSpecWidth = widthMeasureSpec;
		this.measureSpecHeight = heightMeasureSpec;
