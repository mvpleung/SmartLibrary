...
public class CustomTextView extends CheckedTextView {
    ...
    final int Default_TextSize = 15, Default_SubTextSize = 14, Default_Zero = 0;
    ...
    public CustomTextView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mDrawableCenter = a.getBoolean(R.styleable.CustomTextView_ctv_drawableCenter, false);
        mBoundWidth = a.getDimensionPixelSize(R.styleable.CustomTextView_ctv_boundWidth, getWidth());
        mBoundHeight = a.getDimensionPixelSize(R.styleable.CustomTextView_ctv_boundHeight, getHeight());
        mTextSize = a.getDimensionPixelSize(R.styleable.CustomTextView_android_textSize, Default_TextSize);
        mSubTextSize = a.getDimensionPixelSize(R.styleable.CustomTextView_ctv_subTextSize, Default_SubTextSize);
        mText = a.getText(R.styleable.CustomTextView_android_text);
        mSubText = a.getText(R.styleable.CustomTextView_ctv_subText);
        mTextColor = a.getColorStateList(R.styleable.CustomTextView_android_textColor);
        mSubTextColor = a.getColorStateList(R.styleable.CustomTextView_ctv_subTextColor);
        mCheckMarkBound = a.getDimensionPixelSize(R.styleable.CustomTextView_ctv_checkMarkBound, Default_Zero);
     ...
     * set Drawalbe
     *
     * @param mDrawables [left,top,right,bottom]
     */
    ...
    public void setCompoundDrawableTop(Drawable mDrawable, int mBoundWidth, int mBoundHeight) {
        setBound(mDrawable, mBoundWidth, mBoundHeight);
    ...
    private void setBound(Drawable mDrawable, int mBoundWidth, int mBoundHeight) {
        if (mDrawables == null)
            mDrawables = getCompoundDrawables();
        if (mBoundWidth > 0)
            this.mBoundWidth = mBoundWidth;
        if (mBoundHeight > 0)
            this.mBoundHeight = mBoundHeight;
        mDrawable.setBounds(0, 0, this.mBoundWidth, this.mBoundHeight);
    }

    /**
     * DrawableCenter
     *
     * @param mDrawableCenter
     */
