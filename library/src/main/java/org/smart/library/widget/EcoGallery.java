 ...
 * 重构Gallery,解决内存优化
 *
 * @author falnatsheh LiangziChao Update Zoom,Alpha
 */
...
public class EcoGallery extends EcoGalleryAbsSpinner implements GestureDetector.OnGestureListener {
    ...
    /**
     * The Scale of items that are not selected.
     */
    ...
    float gTouchStartX, gTouchStartY;
    ...
    public EcoGallery(Context context, AttributeSet attrs, int defStyle) {
        ...
        int actionDistance = a.getInt(R.styleable.EcoGallery_actionDistance, ACTION_DISTANCE_AUTO);
        setActionDistance(actionDistance);

        boolean onePage = a.getBoolean(R.styleable.EcoGallery_slideOnePage, false);
        setSlideOnePage(onePage);

        float unselectedAlpha = a.getFloat(R.styleable.EcoGallery_unselectedAlpha, 0.5f);
        setUnselectedAlpha(unselectedAlpha);

        float unselectedScale = a.getFloat(R.styleable.EcoGallery_unselectedScale, 1.0f);
        setUnselectedScale(unselectedScale);

        float scaleDownGravity = a.getFloat(R.styleable.EcoGallery_scaleDownGravity, 1.0f);
        setScaleDownGravity(scaleDownGravity);
     ...
     * @param onePage
     */
    public void setSlideOnePage(boolean onePage) {
        mSlideOnePage = onePage;
    }

    public boolean isSlideOnePage() {
        return mSlideOnePage;
    }
    ...
    /**
     * Sets the Scale of items that are not selected in the Gallery.
     *
     * @param unselectedScale the Scale for the items that are not selected.
     *                        attr ref android.R.styleable#Gallery_unselectedScale
     */
    public void setUnselectedScale(float unselectedScale) {
        mUnselectedScale = unselectedScale;
    }

    public float getUnselectedScale() {
        return mUnselectedScale;
    }
    ...
    protected boolean getChildStaticTransformation(View child, Transformation t) {
        ...
        // Zoom.
        if (this.mUnselectedScale != 1) {
            final float zoomAmount = (this.mUnselectedScale - 1) * Math.abs(effectsAmount) + 1;
    ...
    private void detachOffScreenChildren(boolean toLeft) {
        ...
        if (toLeft) {
            ...
            for (int i = 0; i < numChildren; i++) {
                ...
                if (child.getRight() >= galleryLeft) {
                    ...
                    count++;
                    mRecycler.put((isLoopMode() ? firstPosition % adapterCount : firstPosition) + i, child);
                }
            }
        } else {
        ...
        // if (!isLoopMode())
        // mRecycler.put(firstPosition, getChildAt(firstPosition));
    ...
    private View makeAndAddView(int position, int offset, int x, boolean fromLeft) {
        ...
        int adapterCount = getAdapterCount();
        int index = isLoopMode() && adapterCount > 0 ? position % adapterCount : position;
        child = mRecycler.get(index);
     ...
     *
     * @param gravity attr ref android.R.styleable#Gallery_gravity
     */
    ...
    public interface EcoGalleryMode {

        /**
         * 模式（无限循环/正常） EcoGallery.LOOP/EcoGallery.Normal
         *
         * @return
         */
